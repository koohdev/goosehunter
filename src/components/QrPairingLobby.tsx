...
import { Smartphone, MousePointer, QrCode, ShieldCheck } from 'lucide-react';
...
export const QrPairingLobby: React.FC<QrPairingLobbyProps> = ({
  ...
  return (
    <div className="relative z-20 max-w-2xl w-full mx-auto bg-zinc-950/95 border-2 border-zinc-700 rounded-xl p-8 shadow-2xl backdrop-blur-md text-zinc-200 font-mono text-center">
      {/* Title & Badge */}
      <div className="flex items-center justify-center gap-2 mb-2">
        <span className="bg-zinc-800 text-zinc-300 border border-zinc-700 font-semibold text-xs px-3 py-1 rounded-full uppercase tracking-wider">
          Web Arcade Light Gun
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center bg-zinc-900/90 border border-zinc-800 rounded-lg p-6 mb-6">
        {/* QR Code Container */}
        <div className="flex flex-col items-center justify-center p-3 bg-white rounded shadow-sm">
          {qrDataUrl ? (
          ...
          <span className="text-[10px] text-zinc-700 font-semibold mt-1 uppercase">
        ...
        <div className="text-left flex flex-col gap-3.5 text-xs text-zinc-300">
          ...
          <div className="mt-1 p-2.5 bg-black/60 border border-zinc-800 rounded text-center">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 border-t border-zinc-800">
        <div className="flex items-center gap-2 text-xs">
          {controllerConnected ? (
            <span className="flex items-center gap-1.5 text-zinc-200 font-semibold">
              <ShieldCheck className="w-4 h-4 text-emerald-400" /> Controller Connected. Complete calibration on phone.
            </span>
          ) : (
        ...
        <button
          ...
          className="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-bold text-xs px-4 py-2.5 rounded-lg border border-zinc-600 shadow transition active:scale-95"
        >
